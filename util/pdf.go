...
func (p *PDFGenerator) GeneratePdfExpensePlans(expensePlans []entity.ExpensesPlan) (string, error) {
	p.PDF.AddPage()
	...
	// Add data table headers
	headers := []string{"No", "Title", "Description", "Amount", "Date"}
	p.PDF.SetFont("Arial", "", 14)
	for _, header := range headers {
		if header == "No" {
			p.PDF.CellFormat(10, 10, header, "1", 0, "C", false, 0, "")
			continue
		}
		if header == "Description" {
			p.PDF.CellFormat(55, 10, header, "1", 0, "C", false, 0, "")
			continue
		}
		p.PDF.CellFormat(40, 10, header, "1", 0, "C", false, 0, "")
	}
	...
	for i, expenses := range expensePlans {
		data = append(data, []string{
			fmt.Sprintf("%d", i+1),
			expenses.Title,
			expenses.Description,
			fmt.Sprintf("%d", expenses.Amount),
			expenses.Date,
		})
	}
	...
	for _, row := range data {
		for i, col := range row {
			if i == 0 {
				p.PDF.CellFormat(10, 10, col, "1", 0, "C", false, 0, "")
				continue
			}
			if i == 2 {
				p.PDF.CellFormat(55, 10, col, "1", 0, "C", false, 0, "")
				continue
			}
			p.PDF.CellFormat(40, 10, col, "1", 0, "C", false, 0, "")
		}
	...
	fileName := fmt.Sprintf("public/expenses_plans/%s.pdf", uuid.NewString())
