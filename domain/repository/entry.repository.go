...
func (e *entryRepositoryImpl) FindAll(db *gorm.DB, request *request.SearchPaginationRequest, accountID int64) ([]entity.Entry, int64, error) {
	...
	err = db.Model(&entity.User{}).Scopes(e.FilterEntries(request)).Count(&total).Error
	if err != nil {
		return nil, 0, err
	}
...
func (e *entryRepositoryImpl) FilterEntries(request *request.SearchPaginationRequest) func(tx *gorm.DB) *gorm.DB {
	return func(tx *gorm.DB) *gorm.DB {

		if keyword := request.Keyword; keyword != "" {
			keyword = "%" + keyword + "%"
			tx = tx.Where("date LIKE ?", keyword)
		}
